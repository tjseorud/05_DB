 ...
 *  가입일			:ENROLL_DATE	DATE ->기본값을 SYSDATE로 지정
 * */
CREATE TABLE "MEMBER"(
...
INSERT INTO "MEMBER" VALUES('MEM01','123ABC','홍길동','990226-1235467',SYSDATE);
...
-- 추가 샘플 데이터 삽입
-- 가입일 -> SYSDATE를 활용
INSERT INTO "MEMBER" VALUES('MEM02','QWER1234','김미영','981122-2345687',SYSDATE);

-- 가입일 -> DEFAULT 활용(테이블 생성 시 정의된 값이 반영됨)
INSERT INTO "MEMBER" VALUES('MEM03','1Q2W3E4R','박철수','970101-1765432',DEFAULT);
-->ENROLL_DATEZ 컬럼에 설정된 DEFAULT 값(SYSDATE) 삽입

-- 가입일 -> INSERT 시 미작성 하는 경우 -> DEAFULT 값이 반영됨
INSERT INTO "MEMBER"(MEMBER_ID,MEMBER_PW,MEMBER_NAME,MEMBER_SSN) 
VALUES('MEM04','QQQQ2222','신짱구','990304-1111322');
...
/*
    ...
    데이터 무결성 보장을 목적으로 함.
    (데이터 무결성 :결점 없는 데이터, 신뢰도가 높은 데이터 -->중복 X)
...
/*CREATE TABLE 수행시 제약조건을 설정하는 방법
 * 1)컬럼레벨 설정 :테이블 컬럼을 선언할때 제약조건 추가
 *  컬럼명 자료형 [CONSTRAINT 제약조건명] 제약조건
 * 
 * 2)테이블레벨 설정 :테이블 생성 구문 제일 밑에 작성해서 제약조건 추가
...
CREATE TABLE USER_USED_UK(
  USER_NO NUMBER,
--  USER_ID VARCHAR2(20) CONSTRAINT USER_ID_U UNIQUE,	-- 컬럼레벨로 UNIQUE 설정   
	USER_ID VARCHAR2(20) ,  
  ...
  PHONE VARCHAR2(30),
  EMAIL VARCHAR2(50),
	CONSTRAINT USER_ID_U UNIQUE(USER_ID)	-- 테이블 레벨로 USER_ID 컬럼에 UNIQUE 제약조건 지정
);
...
-- 테이블에 대한 식별자(IDENTIFIER) 역할을 함 ->주민번호, 회원번호, 주문번호
...
-->FOREIGN KEY가 설정된 컬럼의 값은 외부로 테이블의 값을 저장할수 있다(== 다른테이블에서 사용할수 있다(참조)) 
...
CREATE TABLE USER_GRADE(
  GRADE_CODE NUMBER PRIMARY KEY,	--FK 제약조건으로 참조할수 있는 대상
...
CREATE TABLE USER_USED_FK(
  ...
  PHONE VARCHAR2(30),
  EMAIL VARCHAR2(50),
  GRADE_CODE NUMBER,	--10,20,30 + NULL 만 저장가능
  CONSTRAINT GRADE_CODE_FK1 FOREIGN KEY(GRADE_CODE)
  	REFERENCES USER_GRADE(GRADE_CODE)	--테이블레벨 FK 설정
  --현재 테이블의 GRADE_CODE 컬럼에 저장할 값은
  --USER_GRADE의 테이블 GRADE_CODE 컬럼에 저장된 값만 가능 ==참조
  /*두 테이블에 똑같은 값이 작성되어 데이터 중복으로 보이자만 
...
--ORA-02291: 무결성 제약조건(GRADE_CODE_FK1)이 위배되었습니다- 부모 키가 없습니다
...
--ORA-02292: 무결성 제약조건(GRADE_CODE_FK1)이 위배되었습니다- 자식 레코드가 발견되었습니다
...
--ORA-02290: 체크 제약조건(GENDER_CHECK)이 위배되었습니다

/*CHECK 제약조건은 설정이 간편하지만 
 * 조건을 수정하는데 어려움이 있다
 * ->그래서 조건이 다양한 경우 FK 제약조건을 이용하는 것이 좋다
 * */

-- CHECK 제약 조건은 범위로도 설정 가능.
CREATE TABLE CHECK_TEST(
	TEST_NAME VARCHAR2(20) PRIMARY KEY,
	TEST_SCORE NUMBER CONSTRAINT CHECK_SCORE
...
--ORA-02290: 체크 제약조건(CHECK_SCORE)이 위배되었습니다
...
INSERT INTO USER_TEST VALUES(1,'A','Q','000000-1000000','남','123-4567-8901','','N');
INSERT INTO USER_TEST VALUES(2,'B','W','000000-2000000','여','123-4567-8901','','N');
INSERT INTO USER_TEST VALUES(3,'C','E','000000-2000000','여','123-4567-8901','','N');
INSERT INTO USER_TEST VALUES(4,'D','R','000000-1000000','남','123-4567-8901','','Y');
INSERT INTO USER_TEST VALUES(5,'E','T','000000-1000000','남','123-4567-8901','','N');
...
CREATE TABLE EMPLOYEE_COPY2 AS
	SELECT e.EMP_ID ,e.EMP_NAME ,NVL(d.DEPT_TITLE,'없음')AS DEPT_TITLE ,j.JOB_NAME ,e.SALARY
	FROM EMPLOYEE e 
