...
SELECT EMP_ID,EMP_NAME,NVL(PHONE,'없음')AS PHONE FROM EMPLOYEE;

/*NULL과 산술 연산 수행시 결과는 무조건 NULL*/
...
--EMPLOYEE 테이블에서 사번,이름,전화번호 조회
--단, 전화번호가 없으면(NULL) '없음', 있으면 '010********'형식으로 변경해서 조회
SELECT 
	EMP_ID,
	EMP_NAME,
...
FROM EMPLOYEE;
/*RPAD(문자열, 길이, 바꿀문자)
 * -문자열 전체에 오른쪽을 지정된 길이 만큼 바꿀문자로 변경해서 조회*/

----
/*선택 함수
 * -여러가지 경우에 따라 알맞은 결과를 선택하는 함수(if,switch 비슷)
 * */
...
--EMPLOYEE 테이블에서 직급코드가 
--'J7'인 사원의 급여를 20% 인상,
--'J6'인 사원의 급여를 15% 인상,
--'J5'인 사원의 급여를 10% 인상,
--나머지 사원은 5% 인상
--사원명, 직급코드,기존급여,인상된 급여, 직급코드 오름차순으로 조회
SELECT 
	EMP_NAME,
	JOB_CODE,
	SALARY,
...
--EMPLOYEE 테이블에서 이름,연봉,세금(연봉 * 세율)조회
--세금 내림차순 조회
SELECT 
	EMP_NAME,
	SALARY *12 as"연봉",
 ...
 * (참고)그룹 수가 늘어나면 그룹함수 결과 개수도 증가 ->GROUP BY
 */
...
--부서코드가 'D6'인 사원의 급여합을 조회
SELECT SUM(SALARY) FROM EMPLOYEE WHERE DEPT_CODE ='D6';

----
/*AVG(컬럼명) :그룹의 평균을 반환하는 함수*/
--모든 사원의 급여 평균 조회
SELECT FLOOR(AVG(SALARY))as"급여 평균" FROM EMPLOYEE;

--모든 사원의 급여 평균보다 많이 받는 사원의 이름,급여, 급여 내림차순 조회
SELECT EMP_NAME,SALARY FROM EMPLOYEE
WHERE SALARY >(SELECT AVG(SALARY) FROM EMPLOYEE)
ORDER BY SALARY DESC;
--일반적으로 일반컬럼 + 그룹함수 결과를 혼용 사용 불가
-- ->GROUP BY 또는 서브쿼리

----
/*MAX(컬럼명),MIN(컬럼명) :최대,최소 값 반환*/
--부서코드가 'D6'인 사원 중 제일 높은 급여,제일 낮은 급여 조회
SELECT MAX(SALARY),MIN(SALARY) FROM EMPLOYEE WHERE DEPT_CODE ='D6';

--모든 사원 중 이름순으로 오름차순 정렬했을때 첫번째 사원, 마지막 사원의 이름 조회
 ...
 * 1)COUNT(*)
 *  -조회된 모든 행의 개수를 반환(NULL 포함 행도 개수증가)
 * 2)COUNT(컬럼명)
 *  -지정된 컬럼 값이 NULL인 행을 제외한 나머지 행의 개수 반환
 * 3)COUNT(DISTINCT 컬럼명)
 * 	-컬럼 값이 중복되는 경우를 제외하고 행의 개수 반환(중복은 한번만 카운드)
 * */
--EMPLOYEE 테이블의 모든 행의 개수 조회
SELECT COUNT(*) FROM EMPLOYEE;

--EMPLOYEE 테이블에서 부서코드가 'D5','D6'인 사원의 수 조회
SELECT COUNT(*) FROM EMPLOYEE WHERE DEPT_CODE IN('D5','D6');

--전화번호가 있는 사원의 수 조회
SELECT COUNT(*) FROM EMPLOYEE WHERE PHONE IS NOT NULL;	--방법 1 -COUNT(*)
SELECT COUNT(PHONE) FROM EMPLOYEE;	--방법 2 -COUNT(컬럼명)

--EMPLOYEE 테이블에 존재하는 부서코드의 수를 조회(몇 종류의 부서코드가 존재하는지 확인)
