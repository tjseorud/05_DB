...
SELECT 사번,이름,직급명 FROM V_EMP WHERE 직급명 ='대리';

/*VIEW를 이용한 DML 사용시 문제점*/
--DEPARTMENT테이블 DEPT_COPY2 테이블 생성
...
--ORA-01400: NULL을 ("DEPT_COPY2"."LOCATION_ID") 안에 삽입할 수 없습니다

--V_DCOPY2를 이용하면 LOCATION_ID 컬럼에 ㅑ할수없기 때문에 자동으로 NULL 삽입됨
-->LOCATION_ID는 NOT NULL 제약조건이 설정되어 있기 때문에 NULL 삽입 불가

-->테이블, VIEW 구조를 파익하지 못하고 있으면 VIEW를 이용한 DML 수행시 잦은 오류 발생

/*VIEW WITH READ ONLY 추가
...
-- == NEXTVAL가 한번도 호출된적이 없다

SELECT SEQ_TEST_NO.NEXTVAL FROM DUAL;	--100
SELECT SEQ_TEST_NO.CURRVAL FROM DUAL;	--100(NO ERR)

SELECT SEQ_TEST_NO.NEXTVAL FROM DUAL;	--105,110,115,120 ->5씩 증가
...
WHERE TEST_NAME='짱구';	--150이상 증가 불가(MAXVALUE)
...
--SEQ_TEST_NO의 증가값을 10, MAXVALUE를 200으로 수정
...
/*WORKBOOK 계정으로 접속*/
SELECT COUNT(*) FROM TB_IDX_TEST;	--TEST_NO(PK, INDEX), TEST_ID(NOT NULL)
