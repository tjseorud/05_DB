...
-->컬럼명, 데이터타입, 데이터, NOT NULL만 복사

-- DEPT_COPY 테이블에 PK 추가
ALTER TABLE DEPT_COPY ADD CONSTRAINT DEPT_COPT_PK PRIMARY KEY(DEPT_ID);

-- DEPT_COPY 테이블의 DEPT_TITLE 컬럼에 UNIQUE 제약조건 추가
ALTER TABLE DEPT_COPY ADD CONSTRAINT DEPT_COPT_U UNIQUE(DEPT_TITLE);
...
ALTER TABLE DEPT_COPY DROP CONSTRAINT DEPT_COPT_U;
...
-- ==테이블의 컬럼을 누군가 참조하고 있다

--1과 연결된 2의 FK 제약조건도 같이 삭제
...
ALTER TABLE DEPT_COPY RENAME CONSTRAINT DEPT_COPY_PK TO PKPKPKPK;
