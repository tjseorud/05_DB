--DQL(Date Query Language)데이터 질의 언어(SELECT)
-->DQL은 DML에 포함된 형태
...
-->선택안된 컬럼은 값이 NULL 또는 설정된 기본값이 들어감
...
INSERT INTO EMP_01(
	...
	FROM EMPLOYEE2 e
		LEFT JOIN DEPARTMENT2 d ON (e.DEPT_CODE = d.DEPT_ID)
);
...
UPDATE EMPLOYEE2 SET (SALARY,BONUS) =(
...
-- 구조가 같은 두 개의 테이블을 하나로 합치는 기능(병합).
-- 컬럼 값이 같은 행이 있으면 UPDATE
-- 컬럼 값이 같은 행이 없으면 INSERT됨
...
/*INTO 빠져있음 ? 선언부에 "INTO EMP_M01" 작성되있음*/
INSERT VALUES (EMP_M02.EMP_ID, EMP_M02.EMP_NAME, EMP_M02.EMP_NO,
               ...
               EMP_M02.ENT_DATE, EMP_M02.ENT_YN);	--4행 수정 + 1행 삽입
